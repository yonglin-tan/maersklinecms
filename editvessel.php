	...
	<?php
		...
		if($resultCheck > 0) {
			...
			while($row = mysqli_fetch_assoc($result)) {
				echo '
				<form action="includes/editvessel.inc.php" method="POST">
				<tr>
				<td><input type="text" name="vid" value="'.$row['vessel_id'].'" readonly>  </td>
				<td><input type="text" name="vname" value="'.$row['vessel_name'].'"></td>
				<td><input type="text" name="vspace" value="'.$row['vessel_spaceavailable'].'"></td>
				<td><input type="text" name="vstatus" value="'.$row['vessel_status'].'"></td>
				<td><button class="button button2" type="submit" name="submit">Edit Vessel</button></td>
				</tr>
				</form>';
			}
		}
