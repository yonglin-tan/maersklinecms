...
<?php
		...
		if($resultCheck > 0) {
			...
			while($row = mysqli_fetch_assoc($result)) {
				echo '<form action="includes/itemdelete.inc.php" method="POST">
				<tr>
				<td><input type="text" name="bid" value="'.$row['booking_id'].'" readonly></td>
				<td><input type="text" name="iid" value="'.$row['item_id'].'" readonly></td>
				<td><input type="text" name="iname" value="'.$row['item_name'].'"readonly></td>
				<td><input type="text" name="idesc" value="'.$row['item_desc'].'"readonly></td>
				<td><input type="text" name="iweight" value="'.$row['item_weight'].'"readonly></td>
				<td><input type="text" name="iremarks" value="'.$row['item_remarks'].'"readonly></td>
				<td><button class="button button2" type="submit" name="submit">Delete Item</button></td>
				</tr>
				</form>';
			}
		}else
